...
describe('Chaos: network-layer failures → always returns rule-based', () => {
  const networkErrors = [
    ['ECONNREFUSED', Object.assign(new Error('connect ECONNREFUSED 127.0.0.1:443'), { code: 'ECONNREFUSED' })],
    ...
    ['ENOTFOUND',    Object.assign(new Error('getaddrinfo ENOTFOUND api.anthropic.com'), { code: 'ENOTFOUND' })],
    ['EHOSTUNREACH', Object.assign(new Error('EHOSTUNREACH'), { code: 'EHOSTUNREACH' })],
  ];

  test.each(networkErrors)('%s → returns rule-based strategy without throwing', async (_name, err) => {

...
describe('_prune sliding window (L195-197 killers)', () => {
  test('calls older than windowMs are pruned', async () => {
    const { cb, tick } = make({ windowMs: 5_000, minCalls: 2, errorRateThreshold: 0.5, failureThreshold: 100 });
...
describe('withRetry backoff bounds (L235-236 killers)', () => {
  ...
  test('delay with baseMs=0 and jitter only is in [0, 100)', async () => {
    const delays = [];
    const originalSetTimeout = globalThis.setTimeout;
    const spy = jest.spyOn(globalThis, 'setTimeout').mockImplementation((fn, ms, ...args) => {
      delays.push(ms);
    ...
    spy.mockRestore();
    // jitter = Math.random() * 100 → in [0, 100); baseMs=0 so delay = 0 + jitter
    // If mutant uses / instead of *, jitter would be [0, 0.01) — delays would be <1
    for (const d of delays) {
      expect(d).toBeGreaterThanOrEqual(0);
      expect(d).toBeLessThan(200); // capped by maxMs
    }
  });
});

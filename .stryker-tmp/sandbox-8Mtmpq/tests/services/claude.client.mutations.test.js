...
describe('Metrics call shape (L96/L105/L113 ObjectLiteral killers)', () => {
...
describe('_escJson special-char escaping (L40-41 killers)', () => {
  test('backslash in text does not break JSON body', async () => {
    await analyze('C:\\path\\to\\file meeting');
    const [, opts] = mockApiFetch.mock.calls[0];
    // body must be parseable JSON — _escJson must escape backslashes
    expect(() => JSON.parse(opts.body)).not.toThrow();
  });

  test('double-quote in text does not break JSON body (L41 StringLiteral)', async () => {
    await analyze('book "urgent" meeting');
    const [, opts] = mockApiFetch.mock.calls[0];
    expect(() => JSON.parse(opts.body)).not.toThrow();
    const body = JSON.parse(opts.body);
    // Content should contain the word 'urgent'
    expect(body.messages[0].content).toContain('urgent');
  });

  test('newline in text does not break JSON body (L41 third StringLiteral)', async () => {
